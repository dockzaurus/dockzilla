...
--   1. shared functions
--   2. tables, with no inline foreign keys
--   3. indexes
--   4. foreign keys, added after every table exists and skipped if present
--   5. updated_at triggers
--
-- Foreign keys live at the end so the file is order-independent and re-runnable:
-- PostgreSQL has no ADD CONSTRAINT IF NOT EXISTS, so section 4 checks
-- pg_constraint before adding each one.
--
-- Every table carries two keys. id is the physical bigint, used for insertion
-- ordering and keyset pagination, and it never leaves the process. identifier is
-- the domain key, generated by the domain before insert, and it is what foreign
-- keys reference.
--
-- gen_random_uuid() is built into PostgreSQL 13 and later, so no extension is
-- required. The default is a safety net for manual inserts; the application
-- always supplies identifier explicitly.
